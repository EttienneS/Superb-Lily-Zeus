...
function create() 
{
    ...
    for (var i = 0; i < 1; i++)
    {
        weapons.push(createWeapon('X'))
        weapons.push(createWeapon('Z'))
    }
...
function createWeapon(spriteName)
{
    let weapon = game.add.sprite(game.world.centerX + (Math.random() * 150) - 75, game.world.centerY + (Math.random() * 150) - 75, spriteName);
    game.physics.p2.enable(weapon);
    weapon.body.setCollisionGroup(weaponCg)
    weapon.body.collides(monsterCg, hitMonster, this);
    
    weapon.scale.set(1.5);
...
{    
    if (!gameOver)
    {
        ...
        {
            springs.push(game.physics.p2.createSpring(cursor, weapon, 0, 30, 1))
        });
        isSwinging = true;
    }
}
